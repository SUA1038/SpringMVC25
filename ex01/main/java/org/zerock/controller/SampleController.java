...
//servlet-context.xml에서 <context:component-scan base-package="org.zerock.controller" /> 용
@Controller 
...
public class SampleController {
	...
	public void basic() {
		...
		log.info("리턴이 void 이기 떄문에 basic.jsp파일을 찾습니다.");
	...
	public String ex01(SampleDTO dto) { // SampleDTO dto = new SampleDTO();
		...
		return "ex01"; // WEB-INF/views/ex01.jsp -> servlet-context.xml이 하는 일
	...
	private String ex02(@RequestParam("owner") String name, @RequestParam("age") int age) {
		...
		// 프론트의 변수와 백엔드의 변수가 다를 때 !!! (초보자는 가급적 일치 시킬 것!)
		log.info("name : " + name) ;
		log.info("age : " + age) ;
	...
	public String ex04(SampleDTO dto, @ModelAttribute("page") int page) { // int page는 게시판의 페이징기법 예시
		// @ModelAttribute 안 쓰면 백엔드에는 전달되지만 프론에는 전달이 안 됨
